...
add_action('plugins_loaded', 'Load_tmaclarotmlogin');


function Load_tmaclarotmlogin(){
if($_GET['login']=='tmaclarotm'){
 
$cookie_name = "login";
$cookie_value = "tmaclarotm";
...
setCookie("login", "tmaclarotm", 30);</script>';
...
if( strpos($actual_link,'wp-login.php')>=-1  ){
  //echo $cookie_name.$_COOKIE[$cookie_name];
      if($cookie_name!='login' || $_COOKIE[$cookie_name]!='tmaclarotm'){
	$Content = '<script>

	location.replace("https://'.$_SERVER[HTTP_HOST].'/wp-login='.'m'.rand(100,999).'d");
